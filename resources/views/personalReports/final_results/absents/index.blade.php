        ...
        <div class="col-12 d-flex flex-wrap justify-content-between align-items-center my-3">
            ...
            <div class="col-12 col-md-4 text-md-end text-center">
                ...
                <span class="badge badge-outline-primary">
                           ...
                           عدد الأفراد المتغيبين : {{count($available_players)}}  </span>
                    ...
                    {{route('final_results.absents.reports')}}
                        ...
                        <!---end -->
                            ...
                            <div class="col-md-4">
                                <input class="form-control" type="text" name="search"
                                       placeholder="الاسم / رقم الهوية / رقم الهاتف" value="{{ request('search') }}">
                            </div>
                        ...
                        <div class="d-flex gap-2 mb-4">
                            ...
                            <a href="{{ (request()->addMembertoReportRid  && request()->addMembertoReportRid > 0 ) ?  url()->current() . '?addMembertoReportRid=' . request()->addMembertoReportRid :   url()->current()  }}"
                               class="btn btn-primary"> <i class="ri-arrow-go-back-line"></i>إعادة ضبط </a>
                        </div>
                    </form>
                </div>
            ...
            <div class="card-body">
                     <div class="card border-success mb-3 rounded-3 overflow-hidden">
                        ...
                        <div class="card-body">
                            ...
                            @isset($Edit_report)
                                <form action="{{route('update-report-registered-members_final', $Edit_report->id)."?addMembertoReportRid=" . $Edit_report->id}}"
                                      method="POST"
                                      id="reportForm">
                                    @csrf
                                    @method('put')
                                    ...
                                    @else
                                        <form action="{{route('generate-report-registered-members_final') . '?absents=yes' }}"
                                              method="POST"
                                              id="reportForm">
                                            @csrf
                                    @endisset
                                                ...
                                                @empty
                                                    <tr>
                                                        <td colspan="12" class="text-center text-muted mt-3">
                                                            @if(request()->hasAny(['mgid', 'reg', 'nat', 'club_id', 'weapon_id', 'search', 'gender', 'active', 'date_from', 'date_to', 'reg_club']))
                                                                <p class="mt-3 w-100 text-center m-auto">لا توجد نتائج
                                                                    مطابقة لبحثك.</p>
